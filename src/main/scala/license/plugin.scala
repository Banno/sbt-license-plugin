...
object Plugin extends sbt.Plugin {
  ...
  private def commentedLicenseTextLines(licenseText: String): List[String] = {
    val commentedLines = licenseText.split("\\r?\\n").map { line => if (line == "") " *" else " * " + line }.toList
    ("/*" :: commentedLines ::: " */" :: Nil)
  }
  ...
  private def formatLicenseHeadersTask = Def.task[Unit] {
    formatSources(javaSource in Compile).value
    formatSources(scalaSource in Compile).value
    formatSources(javaSource in Test).value
    formatSources(scalaSource in Test).value
  }

  private def formatSources(sources: SettingKey[File]) =
